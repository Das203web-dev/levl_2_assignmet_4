...
const Navbar = () => {
    ...
    const navLinks = [
        ...
        { id: 3, name: "borrow summary", paths: "borrowSummary" },
    ];

    return (
        <nav className="flex max-w-7xl mx-auto overflow-hidden py-5 justify-between items-center">
            <Link className="text-4xl bg-black text-white p-2 rounded-full ml-5 md:ml-0 z-50" to={'/'}><MdOutlineLocalLibrary></MdOutlineLocalLibrary></Link>
            <ul className={`flex md:static justify-center items-center flex-col md:flex-row md:w-fit w-full h-screen md:h-fit transition-all duration-500 bg-white z-10 gap-5 uppercase absolute ${open ? " top-0 md:flex-row" : "-top-full"}`}>
                {
